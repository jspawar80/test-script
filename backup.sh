...
ssh -v -i $PRIVATE_KEY_PATH -l $BASTION_USER $BASTION_IP "PGPASSWORD=<your-rds-password> pg_dump -h $RDS_HOST -U $RDS_USER -F p -f /tmp/$DUMP_FILE -d $RDS_DB"
...
docker cp $LOCAL_DIR/$DUMP_FILE $DOCKER_CONTAINER:/tmp/
